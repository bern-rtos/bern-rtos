...
const LINKER_SCRIPT_TEMPLATES: [(&str, &str); 6] = [
    ...
    (
        "${CONF_KERNEL_SIZE}",
        &formatcp!("{}", CONF.kernel.memory_size.0),
    ),
...
fn main() {
    ...
    File::create(out.join("bern.x"))
        .unwrap()
        .write_all(process_linker_script(String::from(include_str!("bern.x.in"))).as_bytes())
        .unwrap();
    ...
    for additional in CONF.memory_map.additional.iter() {
        file.push_str(
            format!(
                "
    {} : ORIGIN = 0x{:X}, LENGTH = {}",
                additional.link_name, additional.start_address, additional.size.0
            )
            .as_str(),
        );
    }

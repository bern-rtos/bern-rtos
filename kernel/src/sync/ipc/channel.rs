...
impl IpcChannel {
    ...
    pub fn split<Q, T, const N: usize>(
        self,
        receive_queue: &'static Q,
    ) -> Result<(IpcSender<T>, IpcReceiver<Q>), ChannelError>
    where
        Q: FiFoQueue<T, { N }> + PushRaw,
        T: Copy,
    {
        let id = syscall::ipc_register(receive_queue).map_err(|_e| ChannelError::ChannelClosed)?;
